...
export async function buddyDisplayName(userId: string): Promise<string> {
...
/**
 * Send a visible push notification for a Goal Buddy event. `path` deep-links the
 * tap (web SW + native both read data.path). Best-effort; never throws.
 */
export async function sendBuddyPush(
  userId: string,
  opts: { title: string; body: string; path: string; type: string },
) {
  try {
