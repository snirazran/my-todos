...
When suggesting or implementing changes, if new NPM packages are required, please **do not attempt to install them directly**. Instead, simply include the necessary import statements in the relevant files. I will handle the `npm install` (or `pnpm install`) process myself. Please make sure to explicitly inform me if any new packages were introduced in your proposed changes.
When creating new files, do not create new folders. Instead, inform me if new folders might be useful for organization.
