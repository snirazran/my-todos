...
enum FrogWidgetStore {
    ...
    /// The add button. The extension can't present a composer, so it records
    /// the request and the webview raises its own quick-add sheet on the next
    /// launch — the app's own route, not a URL, which keeps this off the
